...
export class NbCalendarTimeModelService {
  ...
  getFullHours(use12HoursFormat, step): Array<string> {
    const hours: number = use12HoursFormat ? this.HOURS_IN_DAY_ALT : this.HOURS_IN_DAY;
    const res = [];
    const temp = [];
    ...
    range(hours, value => {
      for (let i = 0; i < 60; i += step) {
        if (use12HoursFormat) {
          if (value !== 0) {
            temp.push(`${this.formatToString(value)}:${this.formatToString(i)} PM`);
          }
          if (value !== 12) {
            res.push(`${this.formatToString(value)}:${this.formatToString(i)} AM`);
          }
        } else {
          res.push(`${this.formatToString(value)}:${this.formatToString(i)}`);
        }
      }
    });
    return use12HoursFormat ? [...res, ...temp] : res;
  }
